...
void Widget::Draw(GraphicsContext* gc){
  ...
  //  UpdateMemWidget(X,Y,w,h,gc);
    
    
}
void Widget::UpdateMemWidget(int32_t X, int32_t Y, int32_t w, int32_t h,  GraphicsContext* gc){
    int i = 0;
    for(int row = X; row < X+w; row){
        for(int col = Y; col < Y+h; col++){
            arraywidget[X][Y] = gc->getPixel(row, col);;            
            i++;
            if(i > (row*col)){
                return;
            }
